...
/**
 * Status-change reservation email (approved / declined / cancelled-by-venue).
 *
 * Provided variables:
 ...
 *   $variant            — 'approved' | 'declined' | 'cancelled_by_venue' | 'no_show'
 *   $variant_copy       — { subject, emoji, headline, intro, pill, pill_color }
 *   $site_name, $site_url
 */
...
$is_negative   = in_array( $variant, array( 'declined', 'cancelled_by_venue', 'no_show' ), true );
$header_bg     = $is_negative ? '#1f2937' : $accent;
$decline_reason = isset( $reservation->decline_reason ) ? trim( (string) $reservation->decline_reason ) : '';
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<title><?php echo esc_html( $variant_copy['headline'] ); ?> &mdash; <?php echo esc_html( $event->post_title ); ?></title>
</head>
...
<div style="background:#ffffff;border-radius:16px;overflow:hidden;margin:24px 16px;">
  ...
  <!-- Header -->
  <div style="background:<?php echo esc_attr( $header_bg ); ?>;padding:36px 28px;text-align:center;">
    <h1 style="color:#ffffff;font-size:26px;font-weight:800;margin:0 0 6px;letter-spacing:-0.5px;font-family:'Inter',Arial,sans-serif;">
      <?php echo esc_html( $variant_copy['emoji'] ); ?> <?php echo esc_html( $variant_copy['headline'] ); ?>
    </h1>
    <p style="color:rgba(255,255,255,0.85);font-size:15px;margin:0;font-family:'Inter',Arial,sans-serif;">
      <?php echo wp_kses( $variant_copy['intro'], array() ); ?>
    </p>
  </div>
  ...
  <div style="padding:28px 28px 8px;">
    ...
    <p style="font-size:14px;color:#71717a;margin:0 0 24px;line-height:1.6;">
      Here are the details for your reservation at <strong><?php echo esc_html( $event->post_title ); ?></strong>.
    </p>
    ...
    <div style="background:#ffffff;border:1.5px solid <?php echo $accent_border; ?>;border-radius:16px;padding:20px;margin-bottom:16px;text-align:center;">
      ...
      <table width="100%" cellpadding="0" cellspacing="0" style="font-family:'Inter',Arial,sans-serif;text-align:left;">
        ...
        <tr>
          ...
          <td style="padding:6px 0;font-size:14px;color:<?php echo esc_attr( $variant_copy['pill_color'] ); ?>;font-weight:700;text-align:right;text-transform:uppercase;letter-spacing:0.4px;">
            <?php echo esc_html( $variant_copy['pill'] ); ?>
          </td>
        </tr>
      </table>

      <?php if ( $variant === 'declined' && $decline_reason !== '' ) : ?>
      <div style="margin-top:14px;padding:12px 14px;border-radius:10px;background:#fef2f2;border:1px solid #fecaca;text-align:left;">
        <div style="font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:0.6px;color:#b91c1c;margin-bottom:4px;font-family:'Inter',Arial,sans-serif;">Reason from the venue</div>
        <p style="font-size:13px;color:#7f1d1d;line-height:1.6;margin:0;font-family:'Inter',Arial,sans-serif;"><?php echo nl2br( esc_html( $decline_reason ) ); ?></p>
      </div>
      <?php endif; ?>

      <?php if ( ! empty( $reservation->notes ) && ! $is_negative ) : ?>
    ...
    <?php if ( ! empty( $extras ) && ! $is_negative ) : ?>
    <!-- Submitted answers -->
    ...
    <p style="font-size:12px;color:#9ca3af;margin:20px 0 8px;line-height:1.6;">
      <?php if ( $variant === 'approved' ) : ?>
        Need to cancel? Reply to this email and let the venue know as early as possible.
      <?php elseif ( $variant === 'declined' ) : ?>
        If you&rsquo;d like to try a different time or party size, please reach back out to the venue.
      <?php elseif ( $variant === 'no_show' ) : ?>
        Running late or still planning to come? Reply to this email and the venue will help if there&rsquo;s still room.
      <?php else : ?>
        If this looks like a mistake, please reply to this email so the venue can sort it out.
      <?php endif; ?>
    </p>
  </div>
