...
<div class="ke-event-page ke-event-page-breakout">
    ...
    <div class="ke-event-body">
        ...
        <?php
        // ─── Cumpleaños CTA — renders directly below the ticket section ───
        $bday_on    = get_post_meta( $event_id, '_ke_birthday_enabled', true ) === '1';
        $bday_title = (string) get_post_meta( $event_id, '_ke_birthday_title', true );
        $bday_desc  = (string) get_post_meta( $event_id, '_ke_birthday_description', true );
        $bday_link  = (string) get_post_meta( $event_id, '_ke_birthday_link', true );
        // Fail-safe: render NOTHING unless enabled AND all three fields present.
        if ( $bday_on && $bday_title !== '' && $bday_desc !== '' && $bday_link !== '' ) : ?>
            <div class="ke-content-section ke-bday">
                <button type="button" class="ke-bday-cta" id="ke-bday-open" aria-haspopup="dialog" aria-controls="ke-bday-sheet">
                    <span class="ke-bday-cta-icon" aria-hidden="true">🎂</span>
                    <span class="ke-bday-cta-text"><?php esc_html_e( '¿Cumples este mes? Ver beneficios', 'kiwi-events' ); ?></span>
                </button>
            </div>

            <div class="ke-sheet-overlay ke-bday-overlay" id="ke-bday-overlay"></div>
            <aside class="ke-sheet ke-bday-sheet" id="ke-bday-sheet" role="dialog" aria-modal="true" aria-labelledby="ke-bday-title">
                <div class="ke-sheet-handle"></div>
                <button type="button" class="ke-sheet-close ke-bday-close" id="ke-bday-close" aria-label="<?php esc_attr_e( 'Cerrar', 'kiwi-events' ); ?>">×</button>
                <div class="ke-sheet-header">
                    <div class="ke-sheet-title" id="ke-bday-title">🎂 <?php echo esc_html( $bday_title ); ?></div>
                </div>
                <div class="ke-sheet-body ke-bday-body">
                    <p class="ke-bday-desc"><?php echo nl2br( esc_html( $bday_desc ) ); ?></p>
                    <a class="ke-bday-link" href="<?php echo esc_url( $bday_link ); ?>" target="_blank" rel="noopener noreferrer"><?php esc_html_e( 'Solicitar información', 'kiwi-events' ); ?></a>
                </div>
            </aside>
            <script>
            (function () {
                var openBtn  = document.getElementById('ke-bday-open');
                var sheet    = document.getElementById('ke-bday-sheet');
                var overlay  = document.getElementById('ke-bday-overlay');
                var closeBtn = document.getElementById('ke-bday-close');
